...
std::shared_ptr<pxpk::FontObject> pxpk::ResourceManager::addFont(std::string filepath)
{
	auto it = observers.find(filepath);
	if (it != observers.end())
	{
		std::shared_ptr<pxpk::FontObject> found = std::dynamic_pointer_cast<pxpk::FontObject> ((*it).second.lock());
		if (found)
			return found;
	}

	std::shared_ptr<pxpk::FontObject> newResource = std::make_shared<pxpk::FontObject>(filepath);
	observers[filepath] = newResource;
	return newResource;
}
